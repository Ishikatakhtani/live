...
//     await fetch(`http://localhost:3000/store/${store.id}`, {
//       method: "PUT",
//       headers: { "Content-Type": "application/json" },
...
const ViewProduct = () => {
  const { storeId } = useParams(); // get storeId from route
    ...
    const fetchStoreAndProducts = async () => {
      try {
        ...
        const productRes = await fetch(
          `http://localhost:3000/products?storeId=${storeId}`
        );
        ...
        <ul>
          {products.map((product) => (
            <li key={product.id}>
              ...
              <ul>
                {Object.entries(product.productProperties).map(([key, value]) => (
                  <li key={key}>
